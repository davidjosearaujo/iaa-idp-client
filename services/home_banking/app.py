...
def home(token, user_info):
    try:
        accounts = resource_servers.get_account_info(os.environ['ACCOUNT_INFO_SERVER'], token)
        loans = resource_servers.get_loans(os.environ['LOANS_SERVER'], token)
        for i, account in enumerate(accounts["accounts"]):
            accounts["accounts"][i]["balance"] = resource_servers.get_balance(
                os.environ['PAYMENTS_SERVER'], token, account["iban"]
            )
            accounts["accounts"][i]["movements"] = resource_servers.get_movements(
                os.environ['PAYMENTS_SERVER'], token, account["iban"]
            )

        return render_template(
            "index.html", username=user_info.get("email"), accounts=accounts, loans=loans
        )
    except:
        return render_template("500.html"), 500
...
@authenticate_and_authorize
def transfer(token, user_info):
    try:
        if request.method == "POST" and request.form.get("from") and request.form.get("to") and request.form.get("amount"):
            resource_servers.post_transfer(os.environ['PAYMENTS_SERVER'], token, request.form.get("from"), request.form.get("to"), request.form.get("amount"))
            return redirect("/transfer")
        else:
            accounts = resource_servers.get_account_info(os.environ['ACCOUNT_INFO_SERVER'], token)
            return render_template(
                "transfer.html", username=user_info.get("email"), accounts=accounts
            )
    except:
        return render_template("500.html"), 500
